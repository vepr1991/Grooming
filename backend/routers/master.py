...
async def update_salon(salon_id: str, p: SalonUpdate, uid: Optional[int] = Depends(verify_telegram_data)):
    if uid and not supabase.table("salons").select("id").eq("id", salon_id).eq("telegram_chat_id", uid).execute().data:
...
async def del_svc(sid: str, uid: Optional[int] = Depends(verify_telegram_data)):
    ...
    if uid and svc.data and not supabase.table("salons").select("id").eq("id", svc.data['salon_id']).eq(
            "telegram_chat_id", uid).execute().data:
        raise HTTPException(403)
...
async def update_product(pid: str, p: ProductUpdate, uid: Optional[int] = Depends(verify_telegram_data)):
    ...
    if not prod.data: raise HTTPException(404)
    if uid and not supabase.table("salons").select("id").eq("id", prod.data['salon_id']).eq("telegram_chat_id",
                                                                                            uid).execute().data:
        raise HTTPException(403)

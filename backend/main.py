...
async def h_start(m):
    ...
    await bot.reply_to(m, f"Привет! Я бот для удобной онлайн-записи.\nЖми кнопку ниже 👇", reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith('approve_') or call.data.startswith('reject_'))
async def handle_admin_decision(call):
    ...
    try:
        ...
        if not salon_res.data:
            await bot.answer_callback_query(call.id, "Салон не найден")
            return
        master_chat_id = salon_res.data['telegram_chat_id']
        if action == 'approve':
            supabase.table("salons").update({"is_approved": True}).eq("id", salon_id).execute()
            await bot.edit_message_text(f"✅ Мастер <b>{salon_res.data['name']}</b> одобрен!", call.message.chat.id,
                                  call.message.id, parse_mode="HTML")
            await bot.send_message(master_chat_id, "🎉 <b>Ваш аккаунт подтвержден!</b>\n\nТеперь вы можете принимать записи.",
                             parse_mode="HTML")
        elif action == 'reject':
            await bot.edit_message_text(f"❌ Мастер <b>{salon_res.data['name']}</b> отклонен.", call.message.chat.id,
                                  call.message.id, parse_mode="HTML")
            await bot.send_message(master_chat_id, "😔 К сожалению, ваша регистрация отклонена администратором.")
    except Exception as e:
        logger.error(f"Approval error: {e}")
        await bot.answer_callback_query(call.id, "Ошибка обработки")


# --- ОБРАБОТЧИК ОТМЕНЫ ЗАПИСИ КЛИЕНТОМ ---
@bot.callback_query_handler(func=lambda call: call.data.startswith('cancel_'))
async def handle_client_cancel(call):
    ...
    try:
        ...
        if not res.data:
            await bot.answer_callback_query(call.id, "Запись не найдена")
            return
        ...
        if appt['status'] == 'canceled':
            await bot.answer_callback_query(call.id, "Запись уже отменена", show_alert=True)
            return
        ...
        await bot.answer_callback_query(call.id, "Запись успешно отменена")
        ...
        await bot.answer_callback_query(call.id, "Произошла ошибка при отмене")

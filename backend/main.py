...
app = FastAPI(title="Grooming API", version="3.8", lifespan=lifespan)
...
def send_telegram_notification_task(salon_id: str, data: BookingRequest, start_dt: datetime):
    try:
        ...
        markup.add(types.InlineKeyboardButton("Открыть админку", web_app=types.WebAppInfo(url="https://grooming-react-front.onrender.com/master")))
...
def send_client_notification(appointment_id: str, status_type: str):
    try:
        res = supabase.table("appointments").select("*, salons(name, phone), services(title)").eq("id", appointment_id).single().execute()
        if not res.data: return
        appt = res.data
        tg_user = json.loads(appt["client_tg_user"]) if isinstance(appt["client_tg_user"], str) else appt["client_tg_user"]
        if not tg_user or not tg_user.get("id"): return
        start_dt = datetime.fromisoformat(appt["start_time"].replace("Z", "+00:00"))
        msg = (f"{'✅' if status_type == 'confirmed' else '❌'} <b>Запись { 'подтверждена' if status_type == 'confirmed' else 'отменена'}</b>\n\n"
    ...
    except Exception as e: logger.error(f"Client notify error: {e}")

# --- API Endpoints ---
@app.get("/")
def health_check(): return {"status": "active", "version": "3.8"}
...
async def get_analytics(salon_id: str, tg_user_id: Optional[int] = Depends(verify_telegram_data)):
    ...
    try:
        ...
        res = supabase.table("appointments").select("*, services(id, title, price)").eq("salon_id", salon_id).gte("start_time", start_month).execute()
        apps = res.data or []
        ...
        for app in apps:
            ...
            elif app['status'] == 'canceled': canc += 1
        top_services = sorted(services_usage.values(), key=lambda x: x['count'], reverse=True)[:3]
        daily_stats = [{"date": datetime.strptime(k, "%Y-%m-%d").strftime("%d.%m"), "value": v} for k, v in sorted(daily_map.items())]
        return {"total_revenue": total_rev, "total_appointments": len(apps), "completed_count": comp, "canceled_count": canc, "today_revenue": today_rev, "daily_stats": daily_stats, "top_services": top_services}
...
async def create_booking(data: BookingRequest, background_tasks: BackgroundTasks):
    ...
    if check_overlap(data.salonId, start_dt, start_dt + timedelta(minutes=data.service.duration_minutes)): raise HTTPException(409, "Slot taken")
    payload = {"salon_id": data.salonId, "service_id": data.service.id, "client_name": data.client.name, "client_phone": data.client.phone, "client_tg_user": json.dumps(data.client.telegram_user), "pet_name": data.pet.name, "pet_breed": data.pet.petBreed, "start_time": start_dt.isoformat(), "end_time": (start_dt + timedelta(minutes=data.service.duration_minutes)).isoformat(), "status": "pending"}
...
async def update_status(appointment_id: str, payload: StatusUpdate, background_tasks: BackgroundTasks):
    ...
    if payload.status in ['confirmed', 'canceled']: background_tasks.add_task(send_client_notification, appointment_id, payload.status)
...
async def register_salon(p: SalonCreate):
    ...
    new_s = p.dict()
    new_s.update({"slug": f"s_{p.telegram_chat_id}_{int(time.time())}", "is_active": True, "schedule": json.dumps([{"day": d, "isWorking": d not in ["Сб", "Вс"], "hours": {"start": "10:00", "end": "20:00"}} for d in ["Пн","Вт","Ср","Чт","Пт","Сб","Вс"]])})
...
async def update_salon(salon_id: str, p: SalonUpdate, uid: Optional[int] = Depends(verify_telegram_data)):
    if uid and not supabase.table("salons").select("id").eq("id", salon_id).eq("telegram_chat_id", uid).execute().data: raise HTTPException(403)
...
async def create_svc(p: ServiceCreate, uid: Optional[int] = Depends(verify_telegram_data)):
    if uid and not supabase.table("salons").select("id").eq("id", p.salon_id).eq("telegram_chat_id", uid).execute().data: raise HTTPException(403)
...
async def del_svc(sid: str, uid: Optional[int] = Depends(verify_telegram_data)):
    ...
    if uid and svc.data and not supabase.table("salons").select("id").eq("id", svc.data['salon_id']).eq("telegram_chat_id", uid).execute().data: raise HTTPException(403)
